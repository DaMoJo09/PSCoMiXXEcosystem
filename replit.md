...
- **CYOA Builder:** An interactive fiction engine with story generation, node editing, variables, conditional choices, and per-node background audio (loops during playtest, auto-syncs on navigation, cleaned up on exit).
...
### Landing Page
A marketing landing page at `/landing` and `/welcome` built to convert visitors across three audience segments (students, creators, schools/teachers). Includes: hero with dual CTA (Start Creating + Schools & Teachers), trust bar (COPPA/FERPA/CTE/Chromebook compliance), 6-tool showcase section, "One Story Three Formats" pipeline section explaining the static-comic → motion-comic → interactive-story pipeline, 4-step "How It Works" flow, audience-specific value cards with 5 bullets each, 4-tier pricing preview (Free/Creator/Pro/School), ecosystem flywheel diagram with clickable app cards linking to external properties, trust metrics, dual final CTA, and legal footer branded as Press Start Gaming Inc.
