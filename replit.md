...
### Card Creator
The card creator supports TCG and Sports modes, offering specific card types and fields for each. Sports mode includes 16 sports with various templates and stat fields. It features name arching and print-ready export at 300 DPI for both single cards and packs. The Pack Builder has a TCG/Sports mode toggle — in Sports mode it provides team-level settings (sport, team name, school/organization, season, coach, mascot, team colors with color pickers), sports pack templates (Team Roster, Starting Lineup, Varsity Squad, JV Squad, All-Stars, Senior Night, Championship Set, Custom Team), per-card sports fields (position, jersey #, grade, height, weight, stat line, player bio, photo upload), sports card template application across all cards, "Apply Team Info to All Cards" button, 3-column roster grid preview, position breakdown summary, and a "Print Team Sheet" export that renders all cards in a 3×3 printable grid at 300 DPI for organizations. Switching pack modes normalizes existing cards.
