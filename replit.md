...
- **Visual Novel Creator:** A full engine with scenes, characters, backgrounds, and dialogue editing. Features include typewriter text, scene transitions, tint overlays, a fullscreen playtest mode with VN-style controls, and dual export (JSON project + standalone playable HTML).
- **CYOA Builder:** An interactive fiction engine with story-to-CYOA generation, node editing, and a visual graph view. Features include node color coding, search/filter, stats bar, "quick play from here," a fullscreen preview with typewriter effect and ending tracker, and improved HTML export.
