...
### Cover Editor (Integrated into Comic Creator)
Cover editing is fully integrated into ComicCreator as an inline full-screen overlay panel (`CoverEditorPanel.tsx`). There is no standalone `/creator/cover` route. The cover editor is launched from a "Create Cover" / "Edit Cover" button in the ComicCreator header or from the cover prompt banner. Cover design data is stored in `project.data.coverDesign` (CoverData type), and captured cover images are saved to `project.data.comicMeta.frontCover/backCover`. The server autosave endpoint preserves `coverDesign` during ComicCreator autosaves via merge logic. Features include transformable text elements, extensive editing controls, template system, AI image generation, text effects/arch, image layers, asset library integration, drawing workspace, filters, undo/redo, and print-ready export at 300 DPI. A `canvasCapture.ts` utility handles html2canvas calls with an `onclone` sanitizer for modern CSS colors.
