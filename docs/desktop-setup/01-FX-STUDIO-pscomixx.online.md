# Desktop App Setup — FX Studio (pscomixx.online)

Copy this entire file into the FX Studio Replit chat. The agent will follow these steps exactly to ship a Mac/Windows/Linux desktop installer.
...
- **App name:** FX Studio
- **Bundle identifier:** `com.pscomixx.fxstudio`
- **Live URL the desktop app loads:** `https://pscomixx.online`
- **GitHub repo (assumed):** `DaMoJo09/FXStudio` — confirm with user, change if different
- **Initial release tag:** `desktop-v0.1.0`
- **Tauri category:** `Graphics` (FX/visual effects tool)
...
### `desktop/dist/index.html` (splash — required so Tauri has something to load before redirect)

```html
<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>FX Studio</title>
...
</head><body><div>Loading FX Studio…</div>
<script>window.location.href = "https://pscomixx.online";</script>
</body></html>
```
...
```json
{
  ...
  "productName": "FX Studio",
  "version": "0.1.0",
  "identifier": "com.pscomixx.fxstudio",
  "build": {
    "frontendDist": "../dist"
  },
  "app": {
...
fn main() {
    tauri::Builder::default()
        ...
        .expect("error while running FX Studio");
}
```
    ...
    steps:
      ...
      - name: Build Tauri app
        ...
        with:
          ...
          releaseName: "FX Studio ${{ github.ref_name }}"
          releaseDraft: false
          prerelease: false
          args: ${{ matrix.args }}
```
...
```bash
cd desktop
npx @tauri-apps/cli icon /path/to/fx-logo-1024.png
```
...
This prevents Rollup from failing on Tauri-only packages during the web build. Skip this step if FX Studio doesn't use any Tauri imports in its web code.
...
`https://github.com/DaMoJo09/FXStudio/releases/tag/desktop-v0.1.0`

Outputs:
- macOS: `FX Studio_0.1.0_universal.dmg`
- Windows: `FX Studio_0.1.0_x64-setup.exe` and `FX Studio_0.1.0_x64_en-US.msi`
- Linux: `fx-studio_0.1.0_amd64.deb` and `fx-studio_0.1.0_amd64.AppImage`
...
2. **"category invalid"** → must be exact string from Tauri schema; for FX Studio use `"Graphics"`
3. **macOS "app is damaged"** → user runs `xattr -cr /Applications/FX\ Studio.app` (no codesign cert)
...
> **Desktop:** Tauri 2 thin shell loading https://pscomixx.online. Workflow: `.github/workflows/desktop-release.yml`. Tag `desktop-v*` to release.
