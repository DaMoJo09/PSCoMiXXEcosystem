...
export default function HopCreator() {
  ...
  const handleImportFromProject = useCallback(async (project: Project) => {
    ...
    try {
      ...
      handleUpdateScene(selectedSceneId, {
        assetUrl: thumbnail,
        assetType: isVideo ? "video" : isGif ? "gif" : "image",
      });
  ...
  }, [selectedSceneId, handleUpdateScene]);
                ...
                <div className="flex-1 flex flex-col gap-4">
                      ...
                      <div className="text-center">
                        ...
                        <div className="flex gap-2 mt-2 justify-center">
                          <button
                            ...
                            className="px-3 py-1 text-xs bg-zinc-800 hover:bg-zinc-700 border border-white/10 transition"
                          ...
                          <button
                            ...
                            className="px-3 py-1 text-xs bg-orange-900/40 hover:bg-orange-800/50 border border-orange-500/30 text-orange-400 transition flex items-center gap-1"
                            data-testid="button-import-from-projects"
                          >
                  ...
                  {selectedScene.assetUrl && (
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => setShowProjectPicker(true)}
                        className="px-3 py-1.5 text-xs bg-orange-900/40 hover:bg-orange-800/50 border border-orange-500/30 text-orange-400 transition flex items-center gap-1"
                        data-testid="button-replace-from-projects"
                      >
                        <FolderOpen className="w-3 h-3" /> My Projects
                      </button>
                      <button
          ...
          <div className="bg-zinc-900 border border-white/10 w-full max-w-2xl max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 shrink-0">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <FolderOpen className="w-4 h-4 text-orange-400" /> Import from My Projects
              </h3>
              <button onClick={() => setShowProjectPicker(false)} className="p-1 hover:bg-zinc-800 transition" data-testid="picker-close">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 border-b border-white/10 shrink-0">
              <div className="flex-1 flex items-center bg-zinc-800 border border-white/10 px-2">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent text-xs text-white p-1.5 outline-none"
              ...
              <div className="flex gap-1">
                {[
                  ...
                  <button
                    ...
                    className={`px-2 py-1 text-[9px] transition ${
                      pickerFilter === f.key ? "bg-orange-600 text-white" : "bg-zinc-800 text-zinc-500 hover:bg-zinc-700"
                    }`}
                    data-testid={`picker-filter-${f.key}`}
                  >
                    {f.label}
                  </button>
                ))}
            ...
            <div className="flex-1 overflow-y-auto p-3">
              {(() => {
                const filtered = (allProjects || [])
                  .filter(p => p.type !== "hop")
                  .filter(p => pickerFilter === "all" || p.type === pickerFilter)
                  .filter(p => !pickerSearch || p.title.toLowerCase().includes(pickerSearch.toLowerCase()))
                  .sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
                if (filtered.length === 0) {
                  return (
                    <div className="text-center py-12">
                      <FolderOpen className="w-10 h-10 text-zinc-700 mx-auto mb-2" />
                      <p className="text-xs text-zinc-500">No projects found</p>
                      <p className="text-[10px] text-zinc-600 mt-1">Create content in Comic, Card, VN, or Motion Studio first</p>
                    </div>
                  );
                }
                return (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {filtered.map(project => (
                      <button
                        key={project.id}
                        onClick={() => handleImportFromProject(project)}
                        className="group bg-zinc-800 border border-white/10 hover:border-orange-500/50 transition text-left overflow-hidden"
                        data-testid={`picker-project-${project.id}`}
                      >
                        <div className="aspect-video bg-zinc-950 flex items-center justify-center overflow-hidden">
                          {project.thumbnail ? (
                            <img src={project.thumbnail} alt={project.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                          ) : (
                            <div className="text-center p-2">
                              <ImageIcon className="w-6 h-6 text-zinc-700 mx-auto" />
                              <p className="text-[9px] text-zinc-600 mt-1">No preview</p>
                            </div>
                          )}
                        </div>
                        <div className="p-2">
