...
function UserCard({ profile, onFollowChange }: { profile: UserProfile; onFollowChange: () => void }) {
  ...
  return (
    <div 
      className="group relative bg-black border-4 border-white overflow-hidden transform transition-all duration-200 hover:translate-x-1 hover:-translate-y-1"
      style={{
        clipPath: "polygon(0 0, 100% 0, 100% calc(100% - 12px), calc(100% - 12px) 100%, 0 100%)"
      }}
      data-testid={`user-card-${profile.id}`}
    >
      <div className="absolute top-0 right-0 w-20 h-full bg-red-600 transform skew-x-[-12deg] translate-x-10 opacity-0 group-hover:opacity-100 transition-opacity z-0" />
      
      <div className="relative z-10 flex items-center gap-4 p-4">
        <button
          onClick={() => navigate(`/social/profile/${profile.id}`)}
          className="relative w-16 h-16 shrink-0 overflow-hidden border-2 border-white group-hover:border-red-500 transition-colors"
        ...
        >
          ...
          {profile.level && profile.level > 1 && (
            <div className="absolute bottom-0 right-0 bg-red-600 text-white text-[10px] font-black px-1 border border-black">
              {profile.level}
            </div>
          )}
        </button>
        ...
        <div className="flex-1 min-w-0">
          <button
            onClick={() => navigate(`/social/profile/${profile.id}`)}
            className="font-black text-white uppercase tracking-wide hover:text-red-500 text-left block truncate transition-colors"
          ...
          {profile.creatorClass && profile.creatorClass !== "Rookie" && (
            <div className="inline-flex items-center gap-1 mt-2 bg-white text-black px-2 py-0.5 text-[10px] font-black uppercase transform -skew-x-6">
          ...
          {profile.role === "admin" && (
            <span className="inline-block ml-2 mt-2 text-[10px] font-black bg-red-600 text-white px-2 py-0.5 border border-white transform -skew-x-6">
              STAFF
            </span>
          )}
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {profile.isFollowing ? (
              ...
              <Button
                ...
                className="bg-white text-black border-2 border-white font-black uppercase text-xs tracking-wider hover:bg-red-600 hover:text-white hover:border-red-600 transition-all transform -skew-x-3"
      ...
      <div className="absolute bottom-0 right-0 w-0 h-0 border-l-[12px] border-l-transparent border-b-[12px] border-b-red-600" />
    </div>
  );
}
...
export default function UserSearch() {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="sticky top-0 z-40 bg-black border-b-4 border-red-600 p-4">
      ...
      <div className="max-w-lg mx-auto p-4 relative">
        <div className="relative mb-6">
          ...
          <Input
            ...
            className="bg-zinc-900 border-4 border-white text-white placeholder:text-white/50 pl-12 py-6 text-lg font-bold focus:border-red-600 transition-colors"
        ...
        {searchQuery.length >= 2 ? (
          <div className="mb-4">
            <h2 className="font-black text-xs text-red-500 mb-3 flex items-center gap-2 uppercase tracking-widest">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
            <TabsList className="w-full grid grid-cols-2 bg-zinc-900 border-4 border-white rounded-none h-14">
              <TabsTrigger 
                value="newest" 
                className="data-[state=active]:bg-red-600 data-[state=active]:text-white font-black uppercase tracking-wider text-white/70 data-[state=active]:border-none rounded-none"
              ...
              <TabsTrigger 
                value="suggested" 
                className="data-[state=active]:bg-white data-[state=active]:text-black font-black uppercase tracking-wider text-white/70 data-[state=active]:border-none rounded-none"
        ...
        {isLoading ? (
          <div className="text-center py-12">
            <div className="animate-spin w-10 h-10 border-4 border-red-600 border-t-transparent mx-auto" />
            <p className="text-white/50 mt-4 font-mono text-sm">LOADING...</p>
          </div>
        ) : displayUsers.length === 0 ? (
          <div className="text-center py-12 border-4 border-white/20 bg-zinc-900/50">
            {searchQuery.length >= 2 ? (
              <>
                <Users className="w-16 h-16 mx-auto text-white/20 mb-4" />
                <p className="text-white/70 font-bold uppercase">No users found</p>
              ...
              <>
                ...
                <p className="text-white/70 font-bold uppercase">No members yet</p>
                <p className="text-white/40 text-sm mt-1">Be the first to invite friends!</p>
              </>
            ) : (
              <>
                <Sparkles className="w-16 h-16 mx-auto text-white/20 mb-4" />
                <p className="text-white/70 font-bold uppercase">No suggestions yet</p>
                <p className="text-white/40 text-sm mt-1">Start following creators to get personalized suggestions!</p>
              </>
            )}
          </div>
        ) : (
