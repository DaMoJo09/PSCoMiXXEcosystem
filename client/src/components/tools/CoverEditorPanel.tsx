...
export const defaultCover: CoverData = {
  ...
  titleColor: "#FFFFFF",
  titleSize: 64,
  subtitleFont: "'Impact', sans-serif",
  ...
  hiddenElements: [],
  bannerTransform: { x: 120, y: 0, width: 480, height: 30, rotation: 0, scaleX: 1, scaleY: 1 },
  publisherTransform: { x: 120, y: 0, width: 480, height: 30, rotation: 0, scaleX: 1, scaleY: 1 },
  issueNumberTransform: { x: 8, y: 50, width: 90, height: 30, rotation: 0, scaleX: 1, scaleY: 1 },
  priceBoxTransform: { x: 8, y: 4, width: 50, height: 46, rotation: 0, scaleX: 1, scaleY: 1 },
  titleTransform: { x: 50, y: 120, width: 500, height: 120, rotation: 0, scaleX: 1, scaleY: 1 },
  subtitleTransform: { x: 120, y: 90, width: 360, height: 28, rotation: 0, scaleX: 1, scaleY: 1 },
  taglineTransform: { x: 100, y: 800, width: 400, height: 50, rotation: 0, scaleX: 1, scaleY: 1 },
  authorTransform: { x: 100, y: 860, width: 400, height: 24, rotation: 0, scaleX: 1, scaleY: 1 },
...
export interface CoverEditorPanelProps {
  initialCoverData?: Partial<CoverData>;
  onSave: (coverData: CoverData, coverImages: { frontCover: string; backCover: string }) => Promise<void>;
  onClose: () => void;
  comicTitle?: string;
  comicAuthor?: string;
}
...
export function CoverEditorPanel({ initialCoverData, onSave, onClose, comicTitle, comicAuthor }: CoverEditorPanelProps) {
  const [coverData, setCoverData] = useState<CoverData>(() => {
    const base = { ...defaultCover };
    if (comicTitle) { base.title = comicTitle; base.spineText = comicTitle; }
    if (comicAuthor) base.author = comicAuthor;
    if (initialCoverData) return { ...base, ...initialCoverData };
    return base;
  });
  ...
  const generateEAN13Barcode = useCallback((isbn: string): string => {
    ...
    let bits = "101";
    for (let i = 0; i < 6; i++) bits += parity[i] === "O" ? LEFT_ODD[d[i + 1]] : LEFT_EVEN[d[i + 1]];
    bits += "01010";
    for (let i = 7; i < 13; i++) bits += RIGHT[d[i]];
    bits += "101";
    const barWidth = 1.6;
    const startX = 10;
  ...
  const captureCoverImages = async () => {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const coverImages = await captureCoverImages();
      await onSave(coverData, coverImages);
  ...
  const moveLayerOrder = useCallback((id: string, direction: "front" | "back" | "forward" | "backward") => {
    ...
    if (idx === -1) { order.push(id); updateCover({ elementZOrder: order }); return; }
    let newOrder = [...order];
    if (direction === "front") { newOrder.splice(idx, 1); newOrder.push(id); }
    else if (direction === "back") { newOrder.splice(idx, 1); newOrder.unshift(id); }
    else if (direction === "forward" && idx < order.length - 1) { [newOrder[idx], newOrder[idx + 1]] = [newOrder[idx + 1], newOrder[idx]]; }
    else if (direction === "backward" && idx > 0) { [newOrder[idx], newOrder[idx - 1]] = [newOrder[idx - 1], newOrder[idx]]; }
        ...
        <div className="absolute origin-top-left" style={{ top: 0, left: 0, width: needsScale ? `${designW}px` : '100%', height: needsScale ? `${designH}px` : '100%', transform: needsScale ? `scale(${scale})` : 'none' }}>
          ...
          {coverData.filters.halftone && (
            <div className="absolute inset-0 pointer-events-none z-[5] mix-blend-multiply"
              style={{ backgroundImage: `radial-gradient(circle, rgba(0,0,0,0.3) 25%, transparent 25%)`, backgroundSize: `${coverData.filters.halftoneSize}px ${coverData.filters.halftoneSize}px` }} />
          )}
          ...
          {coverData.filters.vignette && (
            <div className="absolute inset-0 pointer-events-none z-[6]"
              style={{ background: 'radial-gradient(circle, transparent 50%, rgba(0,0,0,0.5) 100%)' }} />
          )}
            ...
            <>
              {!(coverData.hiddenElements || []).includes("master-banner") && coverData.bannerBgColor && coverData.bannerBgColor !== "transparent" && (
                <TransformableElement
                  ...
                  style={layerStyle("master-banner", Math.max((coverData.elementZOrder || []).indexOf("master-banner"), 0) + 2)}
                >
                  <div className="w-full h-full flex items-center justify-between px-2 text-white" style={{ backgroundColor: coverData.bannerBgColor }}>
                    {editingMasterId === "master-publisher" ? (
                      <input autoFocus className="text-xs font-bold bg-transparent outline-none border-b border-white/50 text-white w-1/2"
                    ...
                    {editingMasterId === "master-tagline" ? (
                      <input autoFocus className="text-xs bg-transparent outline-none border-b border-white/50 text-white w-1/2 text-right"
                        value={coverData.tagline} onChange={(e) => updateCover({ tagline: e.target.value })}
                        onBlur={() => setEditingMasterId(null)} onKeyDown={(e) => { e.stopPropagation(); if (e.key === "Enter" || e.key === "Escape") setEditingMasterId(null); }} />
                    ) : (
              ...
              {!(coverData.hiddenElements || []).includes("master-issue") && coverData.issueNumber && (
                <TransformableElement
                  ...
                  locked={false} containerRef={canvasRef} containerScale={scale}
                  style={layerStyle("master-issue", Math.max((coverData.elementZOrder || []).indexOf("master-issue"), 0) + 2)}
                >
                  <div className="w-full h-full flex items-center justify-center">
                    {editingMasterId === "master-issue" ? (
                      <input autoFocus className="text-2xl font-bold bg-transparent outline-none border-b border-white/50 w-full text-center"
                        style={{ color: coverData.titleColor, fontFamily: coverData.titleFont }}
                        value={coverData.issueNumber} onChange={(e) => updateCover({ issueNumber: e.target.value })}
                        onBlur={() => setEditingMasterId(null)} onKeyDown={(e) => { e.stopPropagation(); if (e.key === "Enter" || e.key === "Escape") setEditingMasterId(null); }} />
                    ) : (
                      <span className="text-2xl font-bold cursor-text" style={{ color: coverData.titleColor, fontFamily: coverData.titleFont }}
                        onDoubleClick={(e) => { e.stopPropagation(); setEditingMasterId("master-issue"); }}>{coverData.issueNumber}</span>
                    )}
                  </div>
                </TransformableElement>
              )}
              ...
              <TransformableElement
                ...
                locked={false} containerRef={canvasRef} containerScale={scale}
                style={layerStyle("master-title", Math.max((coverData.elementZOrder || []).indexOf("master-title"), 0) + 2)}
              >
                <div className="w-full h-full flex items-center justify-center text-center">
                  {editingMasterId === "master-title" ? (
                    <input autoFocus className="font-bold uppercase tracking-tight leading-none bg-transparent outline-none border-b-2 border-white/50 w-full text-center"
                    ...
                    <TextElement id="master-title-text" text={coverData.title} fontSize={coverData.titleSize} fontFamily={coverData.titleFont}
                      color={coverData.titleColor} textEffect={coverData.titleEffect as any} textArch={coverData.titleArch}
                      strokeColor={coverData.titleStrokeColor} strokeWidth={coverData.titleStrokeWidth}
                      fontWeight="bold" textTransform="uppercase" isEditing={false}
                      onEditStart={() => setEditingMasterId("master-title")} onEditEnd={() => {}} onChange={() => {}} />
                  )}
                </div>
              </TransformableElement>
              )}
              ...
              <TransformableElement
                ...
                locked={false} containerRef={canvasRef} containerScale={scale}
                style={layerStyle("master-subtitle", Math.max((coverData.elementZOrder || []).indexOf("master-subtitle"), 0) + 2)}
              >
                <div className="w-full h-full flex items-center justify-center text-center">
                  {editingMasterId === "master-subtitle" ? (
                    <input autoFocus className="italic bg-transparent outline-none border-b border-white/50 w-full text-center"
                      style={{ fontFamily: coverData.subtitleFont, color: coverData.subtitleColor, fontSize: `${coverData.subtitleSize}px` }}
                      value={coverData.subtitle} onChange={(e) => updateCover({ subtitle: e.target.value })}
                      onBlur={() => setEditingMasterId(null)} onKeyDown={(e) => { e.stopPropagation(); if (e.key === "Enter" || e.key === "Escape") setEditingMasterId(null); }} />
                  ) : (
                    <TextElement id="master-subtitle-text" text={coverData.subtitle} fontSize={coverData.subtitleSize} fontFamily={coverData.subtitleFont}
                      color={coverData.subtitleColor} textEffect={coverData.subtitleEffect as any} textArch={coverData.subtitleArch}
                      strokeColor={coverData.subtitleStrokeColor} strokeWidth={coverData.subtitleStrokeWidth}
                      fontStyle="italic" isEditing={false}
                      onEditStart={() => setEditingMasterId("master-subtitle")} onEditEnd={() => {}} onChange={() => {}} />
                  )}
                </div>
              </TransformableElement>
              )}
              ...
              <TransformableElement
                ...
                locked={false} containerRef={canvasRef} containerScale={scale}
                style={layerStyle("master-author", Math.max((coverData.elementZOrder || []).indexOf("master-author"), 0) + 2)}
              >
                <div className="w-full h-full flex items-center justify-center text-center">
                  {editingMasterId === "master-author" ? (
                    <input autoFocus className="font-medium tracking-widest uppercase bg-transparent outline-none border-b border-white/50 w-full text-center"
                    ...
                    <TextElement id="master-author-text" text={coverData.author} fontSize={coverData.authorSize} fontFamily={coverData.authorFont}
                      color={coverData.authorColor} textEffect={coverData.authorEffect as any} textArch={coverData.authorArch}
                      strokeColor={coverData.authorStrokeColor} strokeWidth={coverData.authorStrokeWidth}
                      fontWeight="bold" textTransform="uppercase" isEditing={false}
                      onEditStart={() => setEditingMasterId("master-author")} onEditEnd={() => {}} onChange={() => {}} />
                  )}
                </div>
              </TransformableElement>
              )}
          ...
          {view === "back" && (
            <>
              ...
              <TransformableElement
                ...
                locked={false} containerRef={canvasRef} containerScale={scale}
                style={layerStyle("master-back-title", Math.max((coverData.elementZOrder || []).indexOf("master-back-title"), 0) + 2)}
              >
                <div className="w-full h-full flex items-center justify-center text-center">
              ...
              <TransformableElement
                ...
                locked={false} containerRef={canvasRef} containerScale={scale}
                style={layerStyle("master-blurb", Math.max((coverData.elementZOrder || []).indexOf("master-blurb"), 0) + 2)}
              >
                <div className="w-full h-full flex items-center justify-center text-center p-4">
                  {editingMasterId === "master-blurb" ? (
                    <textarea autoFocus className="leading-relaxed bg-transparent outline-none border border-white/30 w-full h-full resize-none p-2 text-center"
                      style={{ fontFamily: coverData.backBlurbFont, color: coverData.backBlurbColor, fontSize: `${coverData.backBlurbSize}px` }}
                      value={coverData.backBlurb} onChange={(e) => updateCover({ backBlurb: e.target.value })}
                      onBlur={() => setEditingMasterId(null)} onKeyDown={(e) => { e.stopPropagation(); if (e.key === "Escape") setEditingMasterId(null); if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); setEditingMasterId(null); } }} />
                  ) : (
                    <p style={{ fontFamily: coverData.backBlurbFont, color: coverData.backBlurbColor, fontSize: `${coverData.backBlurbSize}px` }}
                      className="leading-relaxed cursor-text"
                      onDoubleClick={(e) => { e.stopPropagation(); setEditingMasterId("master-blurb"); }}>{coverData.backBlurb}</p>
                  )}
                </div>
              </TransformableElement>
              )}
              ...
              <TransformableElement
                ...
                locked={false} containerRef={canvasRef} containerScale={scale}
                style={layerStyle("master-back-author", Math.max((coverData.elementZOrder || []).indexOf("master-back-author"), 0) + 2)}
              >
                <div className="w-full h-full flex items-center justify-center text-center">
              ...
              <TransformableElement
                ...
                locked={false} containerRef={canvasRef} containerScale={scale}
                style={layerStyle("master-back-publisher", Math.max((coverData.elementZOrder || []).indexOf("master-back-publisher"), 0) + 2)}
              >
                <div className="w-full h-full flex items-center justify-center text-center">
              ...
              <div className="absolute bottom-4 left-0 right-0 flex justify-center z-10">
                {coverData.isbn && generateEAN13Barcode(coverData.isbn) ? (
                  ...
                  <div className="w-32 h-10 bg-white/10 flex items-center justify-center text-xs text-white/50 font-mono">ISBN</div>
                )}
              </div>
            </>
          )}
          ...
          {view === "spine" && (
            <TransformableElement
              ...
              locked={false} containerRef={canvasRef} containerScale={scale} minWidth={20} minHeight={50}
              style={layerStyle("master-spine", 2)}
            >
              <div className="w-full h-full flex items-center justify-center">
                {editingMasterId === "master-spine" ? (
                  <input autoFocus className="text-lg font-bold tracking-widest uppercase bg-transparent outline-none border border-white/30 text-center"
                    style={{ fontFamily: coverData.spineFont, color: coverData.spineColor, writingMode: "vertical-rl", textOrientation: "mixed" }}
                    value={coverData.spineText} onChange={(e) => updateCover({ spineText: e.target.value })}
                    onBlur={() => setEditingMasterId(null)} onKeyDown={(e) => { e.stopPropagation(); if (e.key === "Enter" || e.key === "Escape") setEditingMasterId(null); }} />
                ) : (
                  <p style={{ fontFamily: coverData.spineFont, color: coverData.spineColor, writingMode: "vertical-rl", textOrientation: "mixed" }}
                    className="text-lg font-bold tracking-widest uppercase cursor-text"
                    onDoubleClick={(e) => { e.stopPropagation(); setEditingMasterId("master-spine"); }}>{coverData.spineText} — {coverData.author}</p>
                )}
              </div>
            </TransformableElement>
          )}
          ...
          {(() => {
            ...
            return orderedIds.map((id) => {
              ...
              const zIdx = (globalIdx >= 0 ? globalIdx : zOrder.length) + 2;
              const imgLayer = imgMap.get(id);
              if (imgLayer) {
                return (
                  <TransformableElement key={imgLayer.id} id={imgLayer.id}
                    initialTransform={imgLayer.transform} isSelected={selectedLayerIds.includes(imgLayer.id)}
                    ...
                    locked={imgLayer.locked} containerRef={canvasRef} containerScale={scale} style={layerStyle(imgLayer.id, zIdx)}>
              ...
              if (layer) {
                return (
                  <TransformableElement key={layer.id} id={layer.id}
                    initialTransform={layer.transform} isSelected={selectedLayerIds.includes(layer.id)}
                    onSelect={(lid) => handleShiftSelect(lid)}
                    onTransformChange={(lid, transform) => updateTextLayer(view, lid, { transform })}
                    onDelete={(lid) => deleteTextLayer(view, lid)}
                    locked={layer.locked} containerRef={canvasRef} containerScale={scale} style={layerStyle(layer.id, zIdx)}>
                    <TextElement id={layer.id} text={layer.text} fontSize={layer.fontSize} fontFamily={layer.fontFamily}
                      color={layer.color} textArch={layer.textArch} textEffect={layer.textEffect as any}
                      strokeColor={layer.strokeColor} strokeWidth={layer.strokeWidth} shadowColor={layer.shadowColor}
                      fontWeight={layer.fontWeight as any} fontStyle={layer.fontStyle as any} textTransform={layer.textTransform as any}
                      isEditing={editingTextId === layer.id}
                      onEditStart={() => setEditingTextId(layer.id)} onEditEnd={() => setEditingTextId(null)}
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-zinc-950 text-white" data-testid="cover-editor-panel">
      <header className="h-14 border-b border-zinc-800 flex items-center justify-between px-6 bg-zinc-900 flex-shrink-0">
        ...
        <div className="flex gap-2 items-center">
          ...
          {selectedLayerId && (
            <div className="flex border border-zinc-700 mr-1">
              <button onClick={() => alignElements("left")} className="p-1.5 hover:bg-zinc-800 text-zinc-400 hover:text-white" title="Align Left"><AlignHorizontalJustifyStart className="w-3.5 h-3.5" /></button>
              <button onClick={() => alignElements("center-h")} className="p-1.5 hover:bg-zinc-800 text-zinc-400 hover:text-white" title="Align Center"><AlignHorizontalJustifyCenter className="w-3.5 h-3.5" /></button>
              <button onClick={() => alignElements("right")} className="p-1.5 hover:bg-zinc-800 text-zinc-400 hover:text-white" title="Align Right"><AlignHorizontalJustifyEnd className="w-3.5 h-3.5" /></button>
              <button onClick={() => alignElements("top")} className="p-1.5 hover:bg-zinc-800 text-zinc-400 hover:text-white" title="Align Top"><AlignVerticalJustifyStart className="w-3.5 h-3.5" /></button>
              <button onClick={() => alignElements("center-v")} className="p-1.5 hover:bg-zinc-800 text-zinc-400 hover:text-white" title="Align Middle"><AlignVerticalJustifyCenter className="w-3.5 h-3.5" /></button>
              <button onClick={() => alignElements("bottom")} className="p-1.5 hover:bg-zinc-800 text-zinc-400 hover:text-white" title="Align Bottom"><AlignVerticalJustifyEnd className="w-3.5 h-3.5" /></button>
            </div>
          )}
          {selectedLayerId && (
            <div className="flex border border-zinc-700 mr-1">
              <button onClick={() => moveLayerOrder(selectedLayerId, "front")} className="p-1.5 hover:bg-zinc-800 text-zinc-400 hover:text-white" title="Bring to Front"><ChevronsUp className="w-3.5 h-3.5" /></button>
              <button onClick={() => moveLayerOrder(selectedLayerId, "forward")} className="p-1.5 hover:bg-zinc-800 text-zinc-400 hover:text-white" title="Bring Forward"><ChevronUp className="w-3.5 h-3.5" /></button>
              <button onClick={() => moveLayerOrder(selectedLayerId, "backward")} className="p-1.5 hover:bg-zinc-800 text-zinc-400 hover:text-white" title="Send Backward"><ChevronDown className="w-3.5 h-3.5" /></button>
              <button onClick={() => moveLayerOrder(selectedLayerId, "back")} className="p-1.5 hover:bg-zinc-800 text-zinc-400 hover:text-white" title="Send to Back"><ChevronsDown className="w-3.5 h-3.5" /></button>
            </div>
          )}
          <button onClick={() => setShowGuides(!showGuides)}
            ...
            title="Bleed/Trim/Safe Guides" data-testid="button-toggle-guides">
            <Ruler className="w-4 h-4" />
          </button>
          <button onClick={() => setShowAssetBrowser(true)}
            className="px-4 py-2 bg-violet-600 hover:bg-violet-500 border border-violet-500 text-sm font-bold flex items-center gap-2" data-testid="button-open-assets">
          ...
          <button onClick={handleSave} disabled={isSaving}
            ...
            <Save className="w-4 h-4" /> {isSaving ? "Saving..." : "Save Cover"}
          </button>
          ...
          <button onClick={handlePDFExport}
            className="px-3 py-2 bg-amber-500 text-black text-sm font-bold flex items-center gap-2 hover:bg-amber-400" data-testid="button-export-pdf">
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          {activeSection === "content" && (
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input type="text" value={coverData.title} onChange={(e) => updateCover({ title: e.target.value, spineText: e.target.value })}
              ...
              <div className="space-y-2">
                ...
                <input type="text" value={coverData.subtitle} onChange={(e) => updateCover({ subtitle: e.target.value })}
                  className="w-full bg-zinc-800 border border-zinc-700 p-2 text-sm" />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-zinc-400">Author</label>
                <input type="text" value={coverData.author} onChange={(e) => updateCover({ author: e.target.value })}
                  className="w-full bg-zinc-800 border border-zinc-700 p-2 text-sm" />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-zinc-400">Back Cover Blurb</label>
                <textarea value={coverData.backBlurb} onChange={(e) => updateCover({ backBlurb: e.target.value })}
              ...
              <div className="pt-4 border-t border-zinc-700">
                ...
                <select value={coverData.templateId} onChange={(e) => applyTemplate(e.target.value)}
                  className="w-full bg-zinc-800 border border-zinc-700 p-2 text-sm mb-3">
                  {COVER_TEMPLATES.map(template => (<option key={template.id} value={template.id}>{template.name}</option>))}
                </select>
                <label className="text-xs font-bold uppercase text-zinc-400 mb-2 block">Comic Details</label>
                <div className="space-y-2">
                  <input type="text" placeholder="Issue # (e.g., #1)" value={coverData.issueNumber} onChange={(e) => updateCover({ issueNumber: e.target.value })}
                    className="w-full bg-zinc-800 border border-zinc-700 p-2 text-sm" />
                  <input type="text" placeholder="Publisher Name" value={coverData.publisherName} onChange={(e) => updateCover({ publisherName: e.target.value })}
                    className="w-full bg-zinc-800 border border-zinc-700 p-2 text-sm" />
                  <input type="text" placeholder="Tagline" value={coverData.tagline} onChange={(e) => updateCover({ tagline: e.target.value })}
                    className="w-full bg-zinc-800 border border-zinc-700 p-2 text-sm" />
                  <div className="flex gap-2">
                    <input type="text" placeholder="Price (e.g., 40¢)" value={coverData.priceText} onChange={(e) => updateCover({ priceText: e.target.value })}
                      className="flex-1 bg-zinc-800 border border-zinc-700 p-2 text-sm" />
                    <label className="flex items-center gap-2 text-xs text-zinc-400">
                      <input type="checkbox" checked={coverData.showPriceBox} onChange={(e) => updateCover({ showPriceBox: e.target.checked })} className="w-4 h-4" />Show
                    </label>
                  </div>
                  <input type="text" placeholder="ISBN (13 digits for barcode)" value={coverData.isbn} onChange={(e) => updateCover({ isbn: e.target.value })}
                    className="w-full bg-zinc-800 border border-zinc-700 p-2 text-sm" data-testid="input-isbn" />
                </div>
              </div>
              <div className="pt-4 border-t border-zinc-700">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {GENRE_TEMPLATES.map(template => (
                    <button key={template.id} onClick={() => applyGenreTemplate(template)}
                      className="p-2 text-xs font-medium bg-zinc-800 hover:bg-zinc-700 border border-zinc-600"
                      style={{ borderLeftColor: template.colors[1], borderLeftWidth: 3 }}>{template.name}</button>
                  ))}
                </div>
              </div>
            </div>
          ...
          {activeSection === "style" && (
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <select value={coverData.titleFont} onChange={(e) => updateCover({ titleFont: e.target.value })}
                  className="w-full bg-zinc-800 border border-zinc-700 p-2 text-sm">
                  {FONT_OPTIONS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
                </select>
                <div className="flex gap-2">
                  <input type="color" value={coverData.titleColor} onChange={(e) => updateCover({ titleColor: e.target.value })} className="w-10 h-8 bg-zinc-800 border border-zinc-700 cursor-pointer" />
                  <input type="number" value={coverData.titleSize} onChange={(e) => updateCover({ titleSize: Number(e.target.value) })} className="flex-1 bg-zinc-800 border border-zinc-700 p-1 text-sm text-center" min="12" max="120" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-zinc-400">Subtitle Font</label>
                <select value={coverData.subtitleFont} onChange={(e) => updateCover({ subtitleFont: e.target.value })}
                  className="w-full bg-zinc-800 border border-zinc-700 p-2 text-sm">
                  {FONT_OPTIONS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
                </select>
                <div className="flex gap-2">
                  <input type="color" value={coverData.subtitleColor} onChange={(e) => updateCover({ subtitleColor: e.target.value })} className="w-10 h-8 bg-zinc-800 border border-zinc-700 cursor-pointer" />
                  <input type="number" value={coverData.subtitleSize} onChange={(e) => updateCover({ subtitleSize: Number(e.target.value) })} className="flex-1 bg-zinc-800 border border-zinc-700 p-1 text-sm text-center" min="8" max="60" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-zinc-400">Author Font</label>
                <select value={coverData.authorFont} onChange={(e) => updateCover({ authorFont: e.target.value })}
                  className="w-full bg-zinc-800 border border-zinc-700 p-2 text-sm">
                  {FONT_OPTIONS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
                </select>
                <div className="flex gap-2">
                  <input type="color" value={coverData.authorColor} onChange={(e) => updateCover({ authorColor: e.target.value })} className="w-10 h-8 bg-zinc-800 border border-zinc-700 cursor-pointer" />
                  <input type="number" value={coverData.authorSize} onChange={(e) => updateCover({ authorSize: Number(e.target.value) })} className="flex-1 bg-zinc-800 border border-zinc-700 p-1 text-sm text-center" min="8" max="48" />
                </div>
              </div>
              <div className="pt-4 border-t border-zinc-700">
                ...
                {(["title", "subtitle", "author"] as const).map(field => {
                  ...
                  return (
                    <div key={field} className="space-y-1 mb-3">
                      ...
                      <select value={(coverData[effectKey] as string) || "none"} onChange={(e) => updateCover({ [effectKey]: e.target.value })}
                        className="w-full bg-zinc-900 border border-zinc-600 p-1 text-xs" data-testid={`select-${field}-effect`}>
              ...
              <div className="pt-4 border-t border-zinc-700">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="text-center">
                    <input type="color" value={coverData.frontBgColor} onChange={(e) => updateCover({ frontBgColor: e.target.value })} className="w-full h-10 bg-zinc-800 border border-zinc-700 cursor-pointer" />
                    <span className="text-[10px] text-zinc-500">Front</span>
                  </div>
                  <div className="text-center">
                    <input type="color" value={coverData.spineBgColor} onChange={(e) => updateCover({ spineBgColor: e.target.value })} className="w-full h-10 bg-zinc-800 border border-zinc-700 cursor-pointer" />
                    <span className="text-[10px] text-zinc-500">Spine</span>
                  </div>
                  <div className="text-center">
                    <input type="color" value={coverData.backBgColor} onChange={(e) => updateCover({ backBgColor: e.target.value })} className="w-full h-10 bg-zinc-800 border border-zinc-700 cursor-pointer" />
              ...
              <div className="pt-4 border-t border-zinc-700">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button onClick={() => addTextLayer(activeView === "spread" ? "front" : activeView)}
                    className="p-1 bg-white text-black text-xs flex items-center gap-1" data-testid="button-add-text-layer">
                    <Plus className="w-3 h-3" /> Add
                  </button>
                </div>
                {(() => {
                  const viewKey = activeView === "spread" ? "front" : activeView;
                  const viewLayers = coverData[`${viewKey}Layers` as keyof CoverData] as TextLayer[];
                  return viewLayers.length > 0 && (
                    <div className="space-y-2">
                      {viewLayers.map((layer) => (
                        <div key={layer.id} onClick={() => setSelectedLayerId(layer.id)}
                          ...
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-xs font-bold truncate flex-1" style={{ fontFamily: layer.fontFamily, color: layer.color }}>{layer.text || "Empty"}</span>
                            <button onClick={(e) => { e.stopPropagation(); deleteTextLayer(viewKey, layer.id); }} className="p-0.5 hover:text-red-500"><Trash2 className="w-3 h-3" /></button>
                          </div>
                          {selectedLayerId === layer.id && (
                            <div className="space-y-2 pt-2 border-t border-zinc-700" onClick={(e) => e.stopPropagation()}>
                              <input type="text" value={layer.text} onChange={(e) => updateTextLayer(viewKey, layer.id, { text: e.target.value })}
                                className="w-full bg-zinc-900 border border-zinc-600 p-1.5 text-xs" placeholder="Text content" data-testid="input-layer-text" />
                              <select value={layer.fontFamily} onChange={(e) => updateTextLayer(viewKey, layer.id, { fontFamily: e.target.value })}
                                className="w-full bg-zinc-900 border border-zinc-600 p-1 text-xs" data-testid="select-layer-font">
                                {FONT_OPTIONS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
                              </select>
                              <div className="flex gap-2">
                                <input type="color" value={layer.color} onChange={(e) => updateTextLayer(viewKey, layer.id, { color: e.target.value })}
                                  className="w-8 h-7 bg-zinc-900 border border-zinc-600 cursor-pointer" data-testid="input-layer-color" />
                                <input type="number" value={layer.fontSize} onChange={(e) => updateTextLayer(viewKey, layer.id, { fontSize: Number(e.target.value) })}
                                  className="flex-1 bg-zinc-900 border border-zinc-600 p-1 text-xs text-center" min="8" max="200" data-testid="input-layer-fontsize" />
                              </div>
                              <div className="flex gap-1">
                                <button onClick={() => updateTextLayer(viewKey, layer.id, { fontWeight: layer.fontWeight === "bold" ? "normal" : "bold" })}
                                  className={`px-2 py-1 text-xs font-bold border ${layer.fontWeight === "bold" ? "bg-white text-black border-white" : "border-zinc-600 hover:border-zinc-400"}`} data-testid="button-layer-bold">B</button>
                                <button onClick={() => updateTextLayer(viewKey, layer.id, { fontStyle: layer.fontStyle === "italic" ? "normal" : "italic" })}
                                  className={`px-2 py-1 text-xs italic border ${layer.fontStyle === "italic" ? "bg-white text-black border-white" : "border-zinc-600 hover:border-zinc-400"}`} data-testid="button-layer-italic">I</button>
                                <button onClick={() => updateTextLayer(viewKey, layer.id, { textTransform: layer.textTransform === "uppercase" ? "none" : "uppercase" })}
                                  className={`px-2 py-1 text-xs border ${layer.textTransform === "uppercase" ? "bg-white text-black border-white" : "border-zinc-600 hover:border-zinc-400"}`} data-testid="button-layer-uppercase">AA</button>
                              </div>
                              <div>
                                <label className="text-[10px] text-zinc-500 block mb-0.5">Effect</label>
                                <select value={layer.textEffect || "comic"} onChange={(e) => updateTextLayer(viewKey, layer.id, { textEffect: e.target.value })}
                                  className="w-full bg-zinc-900 border border-zinc-600 p-1 text-xs" data-testid="select-layer-effect">
                                  {["none", "comic", "outline", "3d", "retro", "glow", "neon", "fire", "ice"].map(e => (<option key={e} value={e}>{e.charAt(0).toUpperCase() + e.slice(1)}</option>))}
                                </select>
                              </div>
                              {["outline", "comic", "3d", "retro"].includes(layer.textEffect || "comic") && (
                                <div className="flex gap-2">
                                  <div className="flex-1">
                                    ...
                                    <input type="color" value={layer.strokeColor || "#000000"} onChange={(e) => updateTextLayer(viewKey, layer.id, { strokeColor: e.target.value })} className="w-full h-6 bg-zinc-900 border border-zinc-600 cursor-pointer" />
                                  </div>
                                  <div className="flex-1">
                                    <label className="text-[10px] text-zinc-500">Width</label>
                                    <input type="number" value={layer.strokeWidth || 2} onChange={(e) => updateTextLayer(viewKey, layer.id, { strokeWidth: Number(e.target.value) })} className="w-full bg-zinc-900 border border-zinc-600 p-1 text-xs text-center" min="0" max="10" step="0.5" />
                                  </div>
                                </div>
                              )}
                              <div>
                                <label className="text-[10px] text-zinc-500 flex justify-between"><span>Text Arch</span><span className="text-zinc-600">{layer.textArch || 0}</span></label>
                                <input type="range" min="-100" max="100" step="5" value={layer.textArch || 0}
                                  onChange={(e) => updateTextLayer(viewKey, layer.id, { textArch: Number(e.target.value) })} className="w-full h-1.5 accent-cyan-500" data-testid="input-layer-arch" />
                                <div className="flex justify-between text-[9px] text-zinc-600">
                                  <span>Down</span>
                                  <button onClick={() => updateTextLayer(viewKey, layer.id, { textArch: 0 })} className="text-zinc-400 hover:text-white">Reset</button>
                                  <span>Up</span>
                                </div>
                              </div>
                            </div>
              ...
              {(() => {
                ...
                return (
                  <div className="pt-4 border-t border-zinc-700">
                    ...
                    <div onClick={() => setSelectedLayerId(`bg-${viewKey}`)}
                      className={`p-2 border cursor-pointer flex items-center gap-2 ${selectedLayerId === `bg-${viewKey}` ? "border-cyan-500 bg-zinc-800" : "border-zinc-700 hover:border-zinc-500"}`} data-testid="bg-image-layer-item">
                  ...
                  return imgLayers.length > 0 && (
                    <div className="space-y-2">
                      {imgLayers.map((imgLayer) => (
                        <div key={imgLayer.id} onClick={() => setSelectedLayerId(imgLayer.id)}
                          ...
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <button onClick={(e) => { e.stopPropagation(); deleteImageLayer(viewKey, imgLayer.id); }} className="p-0.5 hover:text-red-500"><Trash2 className="w-3 h-3" /></button>
                          </div>
                          {selectedLayerId === imgLayer.id && (
                            <div className="space-y-2 pt-2 border-t border-zinc-700" onClick={(e) => e.stopPropagation()}>
                              <div>
                                <label className="text-[10px] text-zinc-500 flex justify-between"><span>Opacity</span><span className="text-zinc-600">{Math.round((imgLayer.opacity ?? 1) * 100)}%</span></label>
                                <input type="range" min="0" max="1" step="0.05" value={imgLayer.opacity ?? 1}
                                  onChange={(e) => updateImageLayer(viewKey, imgLayer.id, { opacity: Number(e.target.value) })} className="w-full h-1.5 accent-violet-500" data-testid="input-image-opacity" />
                              </div>
                            </div>
                          )}
                        </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-zinc-400 flex justify-between">
                  ...
                  <button onClick={() => { setAiTarget("front"); setShowAIGen(true); }} className="text-[10px] bg-white text-black px-2 py-0.5 flex items-center gap-1"><Wand2 className="w-3 h-3" /> AI</button>
                </label>
                <div onClick={() => frontInputRef.current?.click()}
                  ...
                  {coverData.frontImage ? (<img src={coverData.frontImage} className="w-full h-full object-cover" />) : (
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-zinc-400 flex justify-between">
                  ...
                  <button onClick={() => { setAiTarget("back"); setShowAIGen(true); }} className="text-[10px] bg-white text-black px-2 py-0.5 flex items-center gap-1"><Wand2 className="w-3 h-3" /> AI</button>
                </label>
                <div onClick={() => backInputRef.current?.click()}
                  className="aspect-[2/3] bg-zinc-800 border border-zinc-700 flex items-center justify-center cursor-pointer hover:border-white relative overflow-hidden">
                  {coverData.backImage ? (<img src={coverData.backImage} className="w-full h-full object-cover opacity-50" />) : (
                    <span className="text-zinc-500 text-xs flex flex-col items-center"><Upload className="w-4 h-4 mb-1" /> Upload</span>
                  )}
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-zinc-400 flex justify-between">
                  <span>Spine</span>
                  <button onClick={() => { setAiTarget("spine"); setShowAIGen(true); }} className="text-[10px] bg-white text-black px-2 py-0.5 flex items-center gap-1"><Wand2 className="w-3 h-3" /> AI</button>
                </label>
                <div onClick={() => spineInputRef.current?.click()}
                  ...
                  {coverData.spineImage ? (<img src={coverData.spineImage} className="w-full h-full object-cover opacity-50" />) : (
              ...
              <div className="pt-4 border-t border-zinc-700 space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center"><span className="text-xs text-zinc-400">Contrast</span><span className="text-xs text-zinc-500">{coverData.filters.contrast}%</span></div>
                  <input type="range" min="0" max="100" value={coverData.filters.contrast} onChange={(e) => updateFilter('contrast', Number(e.target.value))} className="w-full accent-white" />
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between items-center"><span className="text-xs text-zinc-400">Brightness</span><span className="text-xs text-zinc-500">{coverData.filters.brightness}%</span></div>
                  <input type="range" min="0" max="100" value={coverData.filters.brightness} onChange={(e) => updateFilter('brightness', Number(e.target.value))} className="w-full accent-white" />
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between items-center"><span className="text-xs text-zinc-400">Saturation</span><span className="text-xs text-zinc-500">{coverData.filters.saturation}%</span></div>
                  <input type="range" min="0" max="200" value={coverData.filters.saturation} onChange={(e) => updateFilter('saturation', Number(e.target.value))} className="w-full accent-white" />
                </div>
                <div className="grid grid-cols-3 gap-2 pt-2">
                  {([
                    ["grayscale", "Grayscale"], ["sepia", "Sepia"], ["invert", "Invert"],
                    ["blur", "Blur"], ["halftone", "Halftone"], ["grain", "Grain"], ["vignette", "Vignette"]
                    ...
                    <label key={key} className="flex items-center gap-1.5 text-xs text-zinc-400 cursor-pointer">
                      <input type="checkbox" checked={coverData.filters[key as keyof typeof FILTER_PRESETS] as boolean}
                        onChange={(e) => updateFilter(key as keyof typeof FILTER_PRESETS, e.target.checked)} className="w-3 h-3" />{label}
                    </label>
                  ))}
                </div>
                <button onClick={() => updateCover({ filters: { ...FILTER_PRESETS } })}
                  className="w-full py-1.5 text-xs bg-zinc-800 hover:bg-zinc-700 border border-zinc-600">Reset Filters</button>
              </div>
            </div>
          )}
              ...
              <div className="space-y-3" data-testid="layers-panel">
                ...
                {(() => {
                  ...
                  if (selectedLayerId.startsWith("bg-")) {
                    ...
                    if (!bgImg) return null;
                    const bgTransform = (coverData[`${bgView}BgTransform` as keyof CoverData] as any) || (bgView === "spine" ? defaultCover.spineBgTransform : defaultCover.frontBgTransform);
        ...
        <ContextMenu>
              ...
              {selectedLayerId && getMasterElementInfo(selectedLayerId) && (
                <div className="absolute top-2 left-1/2 -translate-x-1/2 z-[70] bg-zinc-900 border border-zinc-700 p-2 flex items-center gap-2 shadow-xl" data-testid="floating-toolbar">
                  {(() => {
                    ...
                    return (
                      <>
                        <select value={(coverData as any)[info.fontKey]} onChange={(e) => updateCover({ [info.fontKey]: e.target.value })}
                          className="bg-zinc-800 border border-zinc-600 text-xs p-1 max-w-[120px]">
                          {FONT_OPTIONS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
                        </select>
                        <input type="number" value={(coverData as any)[info.sizeKey]} onChange={(e) => updateCover({ [info.sizeKey]: Number(e.target.value) })}
                          className="w-14 bg-zinc-800 border border-zinc-600 text-xs p-1 text-center" min="8" max="200" />
                        <input type="color" value={(coverData as any)[info.colorKey]} onChange={(e) => updateCover({ [info.colorKey]: e.target.value })}
                          className="w-7 h-7 bg-zinc-800 border border-zinc-600 cursor-pointer" />
                        <select value={(coverData as any)[info.effectKey] || "none"} onChange={(e) => updateCover({ [info.effectKey]: e.target.value })}
          ...
          <ContextMenuContent className="w-56 bg-zinc-900 border-zinc-700 text-white">
            ...
            <ContextMenuSub>
              ...
              <ContextMenuSubContent className="w-48 bg-zinc-900 border-zinc-700 text-white">
                {COVER_TEMPLATES.slice(0, 6).map(template => (
                  <ContextMenuItem key={template.id} onClick={() => applyTemplate(template.id)} className="hover:bg-zinc-800 cursor-pointer text-xs">{template.name}</ContextMenuItem>
                ))}
              </ContextMenuSubContent>
            </ContextMenuSub>
            <ContextMenuSeparator className="bg-zinc-700" />
            ...
            <ContextMenuItem onClick={() => { setDrawingTarget(activeView === "spread" ? "front" : activeView); setShowDrawing(true); }} className="hover:bg-zinc-800 cursor-pointer">
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60]">
          <div className="bg-zinc-900 border border-zinc-700 p-6 w-[500px]">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-lg flex items-center gap-2"><Wand2 className="w-5 h-5" /> AI Generate {aiTarget} cover</h3>
      ...
      {showDrawing && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[60] p-8">
          <DrawingWorkspace
            ...
            initialData={drawingTarget === "front" ? coverData.frontImage : drawingTarget === "back" ? coverData.backImage : coverData.spineImage}
            onSave={(rasterData) => {
