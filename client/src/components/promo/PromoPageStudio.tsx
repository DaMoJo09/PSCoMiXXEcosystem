...
import { Sparkles, Send, Megaphone, GraduationCap, User as UserIcon, Loader2, X, Building2, ImageIcon, Wand2 } from "lucide-react";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] w-[95vw] h-[90vh] max-h-[90vh] bg-zinc-950 border-zinc-700 text-white flex flex-col" data-testid="dialog-promo-studio">
        ...
        <Tabs value={activeType} onValueChange={(v) => setActiveType(v as PromoType)} className="flex-1 flex flex-col min-h-0">
          ...
          <TabsContent value={activeType} className="flex-1 grid grid-cols-1 md:grid-cols-[280px_minmax(0,1fr)_380px] gap-3 mt-3 min-h-0 overflow-hidden">
            {/* GALLERY */}
            ...
            {/* PREVIEW — read-only render of the chosen template with the
                user's current field values applied. The actual editing
                happens in the Comic Creator after "Add to Comic" — the
                template is materialized into a regular editable spread
                with all the standard panel/text/image/bubble tools. */}
            <div className="flex flex-col min-h-0 gap-2 overflow-hidden">
              {selected ? (
                <>
                  <div className="flex items-center justify-center px-2 py-1.5 bg-amber-950/20 border border-amber-900/40 rounded text-[11px] text-amber-200">
                    <Megaphone className="w-3.5 h-3.5 mr-1.5 text-amber-400" />
                    Preview only — full editing unlocks in your comic after "Add to Comic"
                  </div>
                  ...
                  <p className="text-[10px] text-zinc-500 text-center">
                    Tweak the fields on the right to set defaults, then add to your comic.
                  </p>
                </>
              ) : (
            ...
            {/* SIDE PANEL — template default fields. These pre-fill the
                materialized spread's text/images, but every field is fully
                editable in the Comic Creator after insertion. */}
            <div className="flex flex-col min-h-0 gap-2 overflow-hidden">
              {selected ? (
                <>
                  <div className="text-[11px] text-zinc-500 px-1 pt-1">
                    Set initial values. You can change them inline in the comic.
                  </div>
